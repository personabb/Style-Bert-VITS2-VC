...
def run():
    ...
    if args.repo_id is not None: #Huggingfaceのモデルリポジトリにモデルをアップロードする場合
    ...
    net_g = SynthesizerTrn(
        ...
        mas_noise_scale_initial=mas_noise_scale_initial, #0.01
        noise_scale_delta=noise_scale_delta, #2e-6
    ...
    if utils.is_resuming(model_dir): #指定されたディレクトリパスに再開可能なモデルが存在するかどうかを返す
        if net_dur_disc is not None:
            try:
    ...
    if net_wd is not None:
        ...
        wl = WavLMLoss( #small languages modelを利用して、生成された音声自体を埋め込みかしたのちGANで損失関数を計算するクラス
            hps.model.slm.model,
            net_wd,
    ...
    #epoch_strpは現在（もしくは前回中断時）のエポック数（基本は0）
    #残りの学習ステップ数がdiff？
    pbar = None
    if not args.no_progress_bar:
        pbar = tqdm(
            total=global_step + diff, #global_stepは現在（もしくは前回中断時）の学習ステップ数？（基本は0）
            initial=global_step, #0
            smoothing=0.05,
            file=SAFE_STDOUT, #google colabを利用する場合には標準出力が変わるため、その設定。ローカルで利用する場合は勝手に標準出力になる。
        )
    initial_step = global_step #0

    for epoch in range(epoch_str, hps.train.epochs + 1):
        if rank == 0: #基本的には分散学習をしないのならばこちら
            train_and_evaluate(
                rank,
                local_rank,
                epoch,
        ...
        if epoch == hps.train.epochs:
            ...
            utils.safetensors.save_safetensors(
                ...
                for_infer=True,#ここがTrueだと、posterior encoderの重みが保存されない。
            )
            if hps.repo_id is not None: #huggingfaveにアップロードしない場合は無視で良い。
